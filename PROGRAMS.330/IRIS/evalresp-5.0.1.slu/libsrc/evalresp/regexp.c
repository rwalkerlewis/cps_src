 ...
 *                      other libraries.
 *   10.25.2023 - RBHerrmann SLU - main procedures look like prototype. Previously the 
 *                procedures wer oin the original C format, e.g., instead of
 #                int proc (char *)rr the code was int ptoc ( arr);char *arr
 */
#include <stdio.h>
#include <string.h>
 ...
 */
static char *reg (int paren, int *flagp, evalresp_logger *log)
/* Parenthesized? */
{
  register char *ret;
  register char *br;
 ...
 */
static char *regbranch (int *flagp, evalresp_logger *log)
{
  register char *ret;
  register char *chain;
 ...
 */
static char *regpiece ( int *flagp, evalresp_logger *log)
{
  register char *ret;
  register char op;
  register char *next;
 ...
 */
static char *regatom (int *flagp, evalresp_logger *log)
{
  register char *ret;
  int flags;
  ...
  *flagp = WORST; /* Tentatively. */
  ...
  case '|':
  case ')':
    FAIL ("internal urp", log); /* Supposed to be caught earlier. */
    break;
  case '?':
  case '+':
  case '*':
 ...
 */
int evr_regexec (register regexp *prog, register char *string, evalresp_logger *log)
{
  register char *s;
 ...
 */
/* 0 failure, 1 success */
static int regtry ( regexp *prog, char *string, evalresp_logger *log)
{
  register int i;
  register char **sp;
  register char **ep;
 ...
 */
/* 0 failure, 1 success */
static int regmatch ( char *prog, evalresp_logger *log)
{
 ...
 */
static int regrepeat ( char *p, evalresp_logger *log)
{
