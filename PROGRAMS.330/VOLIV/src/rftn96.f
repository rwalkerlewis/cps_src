...
c     29 JUL 2022 - In corporated Brocher (2005) relation to
c           predict Vp and Density from Vs. This is now the initial
c           state for the inversion. Individual layer values can be
c           changed using menu item 30 which sets the relation in a layer
c           for fixed Vp (0), Vp from initial Vp/Vs (1) and now
c           Vp from thge Brocher Vs relation (equation 9). Also a
c           global reset for fixed Vp (10), Vp from current Vp/Vs (11)
c           or Brocher (12).  Brocher,, T.M. (2005). Empirical relations 
...
c           mantle Vp upto 12 (900 km)
c
c           conversion of Vp to density always used Brocher (2005) modified
c           to use PREM relation for upper mantle
c
c
...
c-----
        ...
        if(.not.(ext)) kerr=ssytem('rftnpr96')
c-----
c     get control parameters
c-----
        call gttmp0(iprog,itot,nf1,nf2,nf34,nf5,nf67,nf10,nfilt,
     1      nup,dlam,qaqb,wref,invcsl,lstinv,twnmin,twnmax,iter,
     ...
     4      rdum1,rdum2,rdum3,rdum4,rdum5)
        nft = nfilt / 2
        if(iprog.ne.2)then
            WRITE(LOT,*)'rftn96 requires a receiver function inversion'
            if(iprog.eq.1)then
            WRITE(LOT,*)'Do surf96 39 to clean up, then rftn96'
            else if(iprog.eq.2)then
            WRITE(LOT,*)'Do rftn96 39 to clean up, then rftn96'
            else if(iprog.eq.3)then
            WRITE(LOT,*)'Do joint96 39 to clean up, then rftn96'
            endif
            STOP
        endif
        if(itot.lt.0)then
...
c-----
C           kerr=ssytem('srfdis96')
C           open(2,file='tmpsrfi.08')
...
c-----
C           if(nf34.ne.0) kerr=ssytem('srfdrl96')
c-----
c     06 JAN 2007 - always run srfdrr96
c-----
C            kerr=ssytem('srfdrr96')
...
C               call jsamat(id,nfilt,i,wref,invcsl,nf1,
C     1             invdep,lstinv,sigv,sigg)
                call ramat(id,nfilt,i,wref,invcsl,nf1,
...
C               kerr=ssytem('srfvph > /dev/null 2>&1' )
...
C           if(nup.eq.2  ) then
C               write(LOT,*)'model has been updated:'
C               write(LOT,*)
C     1         'run: (1) for new partials'
C           else
C               kerr=ssytem('srfgph > /dev/null 2>&1')
C           endif
...
c-----
                ...
                if(ival.ge.1.or.ival.le.m)then
                    if(jval.ge.0.and.jval.le.2)then
                        nf10(ival) = jval
                    else if(jval.ge.10.and.jval.le.12)then
                        do jj=1,m
                           nf10(jj) = mod(jval,10)
                        enddo
                    endif
                ...
                write(LOT,'(i2,a,i2,a/a)')m
     1              ,' layers: 1-',m,
     1               ' Vp fixed(0), Vp/Vs fixed(1), Brocher(2)',
     1               ' Global fix Vp (10), use Vp/Vs (11), Brocher (12)'
                write(LOT,*)'Enter layer'
                read(LIN,*)ival
                if(ival.lt.1.or.ival.gt.m)then
                    write(LOT,*)'layer out of range'
                else
...
C       else if(id.eq.40)then
C           call fquery(
C     1     'Enter minimum velocity dispersion std error',
C     2         sigv,lquiet,i)
C           call fbound(0.0,1000.0,sigv,1.0)
C       else if(id.eq.41)then
C           call fquery(
C     1     'Enter minimum gamma dispersion std error',
C     2         sigg,lquiet,i)
C           call fbound(0.0,1000.0,sigg,1.0)
