...
C               WRITE(6,*)'MMAX:',mmax
C               WRITE(6,*)'ALP:'
C               WRITE(6,*) 1,1, alpsh(1,1)
C               WRITE(6,*) 1,2, alpsh(1,2)
C               WRITE(6,*) 2,1, alpsh(2,1)
C               WRITE(6,*) 2,2, alpsh(2,2)
C               WRITE(6,*)'BET:'
C               WRITE(6,*) 1,1, betsh(1,1)
C               WRITE(6,*) 1,2, betsh(1,2)
C               WRITE(6,*) 2,1, betsh(2,1)
C               WRITE(6,*) 2,2, betsh(2,2)
C         endif
          call botupsh()
          call topdownsh()
c-----
     ...
     2      frefp(NL), frefs(NL)
      ...
      do i=1,mmax
           if(isfluid(i))then
            ...
            call aten(om,0.0,0.0,ka ,kb ,alpha,
...
C     WRITE(6,*)'TOP: isrc,irec,ur,uz,tz,tr:',isrc,irec,ur,uz,tz,tr
          ur = er(irec,1,1)      *cu(1)+ er(irec,1,2)      *cu(2)
     1       + er(irec,1,3)*ep(irec)*cd(1)+ er(irec,1,4)*es(irec)*cd(2)
          uz = er(irec,2,1)      *cu(1)+ er(irec,2,2)      *cu(2)
     1       + er(irec,2,3)*ep(irec)*cd(1)+ er(irec,2,4)*es(irec)*cd(2)
          tz = er(irec,3,1)      *cu(1)+ er(irec,3,2)      *cu(2)
     1       + er(irec,3,3)*ep(irec)*cd(1)+ er(irec,3,4)*es(irec)*cd(2)
          tr = er(irec,4,1)      *cu(1)+ er(irec,4,2)      *cu(2)
     1       + er(irec,4,3)*ep(irec)*cd(1)+ er(irec,4,4)*es(irec)*cd(2)
...
C                 do ii=1,4
C                    do jj=1,4
C                       WRITE(6,*) ii, jj, alppsv(ii,jj)
C                    enddo
C                 enddo
C           endif
C        if(verbose)then
C              WRITE(6,*)'RD11(',mmax,')=',RD(mmax,1,1)
C              WRITE(6,*)'RD12(',mmax,')=',RD(mmax,1,2)
C              WRITE(6,*)'RD21(',mmax,')=',RD(mmax,2,1)
C              WRITE(6,*)'RD22(',mmax,')=',RD(mmax,2,2)
C              WRITE(6,*)'TD11(',mmax,')=',TD(mmax,1,1)
C              WRITE(6,*)'TD12(',mmax,')=',TD(mmax,1,2)
C              WRITE(6,*)'TD21(',mmax,')=',TD(mmax,2,1)
C              WRITE(6,*)'TD22(',mmax,')=',TD(mmax,2,2)
C        endif
      return
      end
...
C                 do ii=1,4
C                    do jj=1,4
C                       WRITE(6,*) ii, jj, betpsv(ii,jj)
C                    enddo
C                 enddo
C           endif
C           if(verbose)then
C              WRITE(6,*)'RU11(',1,')=',RU(1,1,1)
C              WRITE(6,*)'RU12(',1,')=',RU(1,1,2)
C              WRITE(6,*)'RU21(',1,')=',RU(1,2,1)
C              WRITE(6,*)'RU22(',1,')=',RU(1,2,2)
C              WRITE(6,*)'TU11(',1,')=',TU(1,1,1)
C              WRITE(6,*)'TU12(',1,')=',TU(1,1,2)
C              WRITE(6,*)'TU21(',1,')=',TU(1,2,1)
C              WRITE(6,*)'TU22(',1,')=',TU(1,2,2)
C           endif
      return
      end
...
C      WRITE(6,*)'RDRU:',m,i,j
      ...
      enddo
C         WRITE(6,*)'k,RD,RU:',m,i,j,,csum,RD(m,i,k), RU(m,k,j)
      RDRU = eye(i,j) - csum
      WRITE(6,*)m,i,j,RDRU
      return
      end
...
c-----
c     these routine are to evaluate the eigenfunction integrals
c     This will be called as
c     getfv(fv,nua,d) 
c
c     INT_{z_j-1}}^{z_j} exp(- 2 Nu (z - z_j-1) ) dx
c-----
      subroutine getfv(fv,nu,d)
      implicit none
      complex*16 fv, nu
      real d
      if(cdabs(nu*d).eq.0.d+00) then
           fv = dcmplx(1.0d+00,0.0d+00)*d
      else
           fv = (dcmplx(1.0d+00,0.0d+00)-cdexp(-2.*nu*d))/(2.*nu)
      endif
      return
      end

      subroutine getfvab(fv,nu,d)
c-----
c
c     INT_{z_j-1}}^{z_j} exp(-  Nu (z - z_j-1) ) dx
c-----
      implicit none
      complex*16 fv, nu
      real d
      if(cdabs(nu*d).eq.0.d+00) then
           fv = dcmplx(1.0d+00,0.0d+00)
      else
           fv = (dcmplx(1.0d+00,0.0d+00)-cdexp(-nu*d))/(nu)
      endif
      return
      end

      subroutine getgv(gv,nu,d)
      implicit none
      complex*16 gv, nu
      real d

      if(dreal(nu*d).gt.1.0e+20) then
           gv = dcmplx(0.0d+00,0.0d+00)
      else
           gv = d*cdexp(-nu*d)
      endif
      return
