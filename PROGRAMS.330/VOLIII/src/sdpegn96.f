...
c       09 JUN 2015 - added -E flag to plot Rayleigh wave ellipticity
c       09 FEB 2017 - change label from sec to s
c       10 MAY 2018 - changed format label for Love output from ARE to
c                     ALE and ENERGY to AL and AR for Love and Rayleigh
c       28 SEP 2018 - added -ZF to plot the ratio of Z/H. This is just
c                     1/ellipticity
c       13 DEC 2018 - added -AG to display using acoustic gravity codes
c       08 FEB 2020 - updated the -h output
c       16 NOV 2023 - If  kcolor in -K kolor is negative
c                       then plot each mode with a different color
c                       with mode 0 = 1000(reD) and nmode -1 = 1100 (blue)
c       05 FEB 2024 - If there are any zero values in A0 r GAMMA and if 
c                     the y-axis is logarithmic, do not plot
c----------------------------------------------------------------------c
        integer LIN, LER, LOT
...
c-----
        ...
        logical doasc, doerr, doshwmd, doblack, doticonly, dokmonly
        logical verbose, doag
        integer lintyp
...
c-----
     ...
     2      kolor, dotext,iporg,doxlog,dolat,doticonly,dokmonly,
     ...
     4      verbose,doag)
        ...
        if(verbose)then
            WRITE(LOT,*)'ilorr     :', ilorr
            WRITE(LOT,*)'dofreq    :', dofreq
            WRITE(LOT,*)'dobox     :', dobox 
            WRITE(LOT,*)'xmin      :', xmin  
            WRITE(LOT,*)'xmax      :', xmax  
            WRITE(LOT,*)'ymin      :', ymin  
            WRITE(LOT,*)'ymax      :', ymax  
            ...
            WRITE(LOT,*)'xlen      :', xlen    
            WRITE(LOT,*)'ylen      :', ylen    
            WRITE(LOT,*)'width     :', width    
            WRITE(LOT,*)'kolor     :', kolor    
            WRITE(LOT,*)'iporg     :', iporg     
            WRITE(LOT,*)'doxlog    :', doxlog    
            WRITE(LOT,*)'dolat     :', dolat     
            WRITE(LOT,*)'doticonly :', doticonly     
            WRITE(LOT,*)'dokmonly  :', dokmonly     
            WRITE(LOT,*)'doasc     :', doasc        
            WRITE(LOT,*)'dosurf    :', dosurf        
            WRITE(LOT,*)'doswhmd   :', doshwmd        
            WRITE(LOT,*)'lintyp    :', lintyp        
            WRITE(LOT,*)'doblack   :', doblack        
            WRITE(LOT,*)'datfil    :', datfil         
        endif
c------
...
c------
     ...
     1      x0, y0, xlen, ylen, width, doblack,
     2      kkolor,iporg,doxlog,doylog,datfil,doerr,lintyp,
     3      dokmonly, doticonly)
        integer LIN, LER, LOT
        parameter (LIN=5,LOT=6,LER=0)
c-----
...
c       width   R   width of plotted curve
c       kkolor  I*4 CALPLOT pen value - if negative then color is based on mode
c       iporg   I*4 1 phase velocity
c               2 group velocity
c               3 gamma
c               4 A0
c               5 A0 sqrt(c)
c               6 Rayleigh wave ellipticity
...
c-----
        integer*4 ilorr
        logical dofreq, dobox, doxlog, doylog, dokmonly, doticonly
...
c-----
c       if the x-axis is not defined on the command line
c       then use the data set limits
c-----
        if(xmin .lt.0 .or. xmax .lt.0)then
            xmin = tmin
            xmax = tmax
        endif
        if(ymin .lt. -12340. .or. ymax .lt. -12340.)then
            ymin = cmin
            ymax = cmax
        endif
c-----
...
c-----
     ...
     1      doxlog,doylog,dofreq,dokmonly,doticonly)
c-----
c       plot the dispersion curves
c-----
...
c-----
            rewind 1
            call getmdl(1,nmax,d,a,b,rho,qa,qb,
...
c-----
  ...
  500   continue
            ...
            do 300 i=1,kmode
        call mapkolor(kkolor,i-1, modmax,kolor)
        call newpen(kolor)
...
c-----
                ...
                if(iporg.eq.1)then
                    ...
                    v = u
c-----
c     the next three are plot logarithmically
c     so prevent plotting a zero value
c-----
                    ...
                    IF(V.LE.0.0)THEN
                      V=1.0E-35
                    ENDIF
                else if(iporg.eq.6)then 
                    v = rur0
                else if(iporg.eq.7)then 
                    v = 1./rur0
                endif
                xup  = t
                yup  = v
...
c-----
     ...
     1                  y0,ylen,yy,yyp,yym)
                    ...
                    call  mapkolor(kkolor,jmode(i), modmax,kolor)
                    call newpen(kolor)
                    endif
                    if(doerr)then
                        call plot(xx,yym,3)
...
c
     ...
     1      xmn,xmx,ymn,ymx,
     2      doxlog,doylog, dofreq,dokmonly,doticonly)
        parameter (LIN=5,LOT=6,LER=0)

        logical dobox, doxlog, doylog, dofreq,dokmonly,doticonly
        character ylabel*14, xlabel*14
c------
...
c------
        ...
        if(ilorr.eq.1)then
            if(iporg.eq.1)then
                ...
                call pinitf('SLEGNG.PLT')
                ylabel = 'Gamma (1/km)'
            else if(iporg.eq.4)then
                call pinitf('SLEGNA0.PLT')
                ylabel = 'AL'
            else if(iporg.eq.5)then
                call pinitf('SLEGNAC.PLT')
                ylabel = 'AL sqrt(c)'
            endif
        else if(ilorr.eq.2)then
            if(iporg.eq.1)then
                call pinitf('SREGNC.PLT')
                if(dokmonly)then
                    ylabel = 'km/s'
                else
                    ylabel = 'C (km/s)'
                ...
                call pinitf('SREGNG.PLT')
                ylabel = 'Gamma (1/km)'
            else if(iporg.eq.4)then
                call pinitf('SREGNA0.PLT')
                ylabel = 'AR'
            else if(iporg.eq.5)then
                call pinitf('SREGNAC.PLT')
                ylabel = 'AR sqrt(c)'
            else if(iporg.eq.6)then
                call pinitf('SREGNE.PLT')
                ylabel = 'Ellipticity'
            else if(iporg.eq.7)then
                call pinitf('SREGNZR.PLT')
                ylabel = '1/Ellipticity'
            endif
        endif
c-----
 ...
 1001   continue
     ...
     *  'surface85 data file terminates at  perd=',t0
     ...
     4      verbose,doag)
        ...
        logical doasc
        logical doerr,doshwmd, doblack, doticonly, dokmonly
        logical verbose, doag
        integer lintyp
...
c-----
 ...
 1000   continue
     ...
     1              names(1:4).ne.'-LAT')then
            ...
            else if(names(1:8).eq.'-TICONLY')then
                doticonly = .true.
            else if(names(1:7).eq.'-ONLYKM')then
                dokmonly = .true.
 ...
 2000   continue
     ...
     1  '-Ac               AR sqrt(c) or AL sqrt(c) amplitude factor'
        write(LER,*)
     1  '-E                Rayleigh ellipticity (Ur/Uz) -R required'
        write(LER,*)
     1  '-ZR               1/Rayleigh ellipticity (Uz/Ur) -R required'
        write(LER,*)
     ...
     1  '-S         (default ignore) create SLEGN.dsp or SREGN.dsp'
        write(LER,*)
     1  '-TICONLY   (default false) no numbers on y-axis only tics'
        write(LER,*)
     1  '-ONLYKM    (default false) km/s not U(km/s) C(km/s)'
        write(LER,*)
     1  '-LAT       (default false) use output of slat2d96'
        write(LER,*)
     1  '-AG      (default false) for use with acoustic gravity code'
        write(LER,*)
...
c-----
     ...
     1          mname,ipar,fpar)
        if(doshwmd)then
        call tputmdl(2,nmax,d,a,b,rho,qa,qb,
     1          nper,depths,depthr,
     1          mname,ipar,fpar)
        endif
            call gethed(1,ifunc,kmode,t0,ierr)
     ...
     *      'Number of modes at the lowest period:',t0,' = ',kmode
            write(LOT,*) ' '
            write(LOT,*) 'Model:'
            write(LOT,15) (d(i),a(i),b(i),rho(i),qa(i),qb(i),i=1,nmax)
c-----
...
c------
  200       continue
     ...
     3              sumkr,sumgr,sumgv,ierr)
c-----
c      note for Love the rdur is actually TT
c      and  for Rayleigh the rdur is TR and rduz is TZ
c-----
        if(doshwmd)then
...
c-----
   ...
   20   format(' LMODE NFREQ    PERIOD(S) FREQUENCY(Hz)  C(KM/S) ',
     1  '     U(KM/S)       AL          GAMMA(1/KM)   ')
   30   format(' RMODE NFREQ    PERIOD(S) FREQUENCY(Hz)  C(KM/S) ',
     1  '     U(KM/S)       AR          GAMMA(1/KM)  ELLIPTICITY')
c-----
c       open the output file
c-----
            open(2,file=ofile,status='unknown',
...
c-----
 ...
 1000   continue
        ...
        subroutine tputmdl(lun,mmax,d,a,b,rho,qa,qb,nper,dphs,dphr,
     1      mname,ipar,fpar)
        parameter (NL=200)
        ...
        real*4 fpar(10)
            ...
            write(lun,*)fpar
            write(lun,*)mmax
            write(lun,1)(d(i),a(i),b(i),rho(i),
     1          qa(i),qb(i),i=1,mmax)
            write(lun,*)nper,dphs,dphr
    1   format(4f12.4, 2g11.4)
        return
        end
