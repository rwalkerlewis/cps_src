...
c       13 MAY 2019 - add verbose flag to output the values as a function 
c                     of depth for use by other codes
c       05 FEN 2024 - change the legend at bottom of figure from
c                     one line to three for clarity when XLEN is small
c----------------------------------------------------------------------c
        ...
        character ofile*20, dfile*20, vfile*20
        logical ext
c-----
c       command line arguments
c-----
        logical dobox, dotext, doclean, doag, verbose
        integer*4 ilorr
...
c-----
     ...
     1      x0, y0, xlen, ylen, 
     2      kolor,dotext,doclean,doag,verbose)
...
c------
     ...
     1      access='sequential',form='unformatted')
        ...
        if(dotext)then
            call outtxt(ofile,vfile,ilorr,verbose)
        endif

        call outplt( dobox, xmin, xmax, ymin, ymax, 
     1      x0, y0, xlen, ylen, 
     2      kolor,ilorr,doclean,verbose)
...
c-----
        integer*4 ilorr
        logical dobox, doxlog, doylog,verbose
...
c-----
        if(.not.doclean)then
            ...
            call symbol(x0,y0+ylen-0.45,0.10,' MODE=',0.0,6)
            call number(999.0,999.0,0.10,real(jmode-1),0.0,-1)
            call symbol(x0,y0+ylen-0.65,0.10,' C=',0.0,3)
...
c-----
 ...
 2000   continue
     ...
     1  '-X0 x0 -Y0 y0 -K kolor -NOBOX -TXT -CLEAN -? -AG -V -h'
     ...
     1  '-AG      (default false) for use with acoustic gravity code'
        write(LER,*)
     1  '-C         (default false) output depth values in S?EGN.VER'
        write(LER,*)
     ...
     1  '-h         (default false) online help'
        ...
        subroutine outtxt(ofile,vfile, ilorr,verbose)
c-----
...
c-----
     ...
     1          access='sequential',form='formatted')
            rewind 2
c-----
c       open the vebose output file
c-----
            if(verbose)then
                 open(3,file=vfile,status='unknown',
     1               access='sequential',form='formatted')
                 rewind 3
            endif
c-----
  ...
  200   continue
    ...
    1   format(i5,1x,5e11.3)
            if(verbose)then
              ztop = 0.0
              do i=1,nmax
                write(3,3)j-1,i,ztop,ur(i),tur(i),
     1              dcdh(i),dcdb(i),dcdr(i)
                ztop = ztop + d(i)
              enddo
            endif
            else if(ilorr.eq.2)then
    ...
    3   format(2i5,1x,9e11.3)
            if(verbose)then
              ztop = 0.0
              do i=1,nmax
                write(3,3)j-1,i,ztop,ur(i),tur(i),uz(i),tuz(i),
     1              dcdh(i),dcda(i),dcdb(i),dcdr(i)
                ztop = ztop + d(i)
              enddo
            endif
            endif
