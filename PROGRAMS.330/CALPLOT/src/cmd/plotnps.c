...
CHANGES
	...
	18 MAR 2010 - for EPS force an initial background fill
			also permit -I inverted background flag
			only for EPS for use with PNG etc
	23 MAR 2010 - fixed error in printf of bg_fill
	07 APR 2010 - new command to force bg_fill
	10 JUN 2013 - changed initializer 1 setlinejoin to
			2 setlinejoin. For trace plots the 1 setlinejoinn
			caused a jitter that did not look good at high
			magnification. Changed at 2 places
	06 MAY 2014 - correct hue logic starting liunes 892 and 900 (Larry Baker, USGS Menlo Park)
*/
...
int di_gcmdln(int argc,char *argv[]);
...
INT	dc_hardwarefill = 1; 	/* flag to have all filling done by hardware */
				/* = 0 do shading here
				   = 1 do shading here  including seismic
				   = 2 hardware fill but not for seismic -
                                       use hardware polygon fill
				 */
INT	dc_curfont = 0;
...
static void bg_fill(INT lx,INT ly,INT ux,INT uy);
...
static INT	black = 0;	/* 0 paint onto white paper 
				   1 paint onto black paper */
...
static INT 	size_paper = 0;	/* flag for 11x14 paper document 
				 ...
				 */
static INT 	bblx, bbly, bbux, bbuy; /* bounding box coordinates for EPS */
...
void dv_openpl(int showmenu)
{
	if(stream == NULL){
		...
		if(iseps == 1 ){
			...
			/* 18 MAR 2010 for EPS force background fill
				to that transparency can never occur 
				when converting the EPS file to PNG */
			bg_fill(dc_bottom_eps,dc_left_eps,dc_top_eps,dc_right_eps);
...
void dv_erase(INT mode)
{
	if(pagedirty){
		...
		if(dc_linewidth < dc_minlinewidth)
			...
			(void)fprintf(stream,"%d unit setlinewidth\n",dc_linewidth);
		OUTSTR("1 setlinecap 2 setlinejoin newpath\n")
	}
	dc_shdon = 0;
	dc_shdcur = 0;
	pagedirty = 0;
...
int di_gcmdln(int argc,char *argv[])
{
	...
	/* for EPS, we change the rotate flag - this is because
		...
		18 MAR 2010 - permit inverted background only for EPS
	*/
	if(iseps==1){
		if(dc_rotate != 0){
			dc_rotate = 0;
...
static void bg_fill(INT lx,INT ly,INT ux,INT uy)
{
	...
	fprintf(stream,"%d %d m %d %d l %d %d l %d %d l closepath \n",
		lx,ly,ux,ly,ux,uy,lx,uy);
	if(do_bgfill == 1)
		if(black == 0)
			fprintf(stream,"1.000 1.000 1.000 setrgbcolor fill\n");
	if(black == 1)
		fprintf(stream,"0.000 0.000 0.000 setrgbcolor fill\n");
...
fprintf(stderr,"-I          (default off) Invert background (e.g make black) for EPS\n" );
fprintf(stderr,"-BGFILL     (default off) Force a background fill for use with ImageMagick or GraphicsMagick\n");
fprintf(stderr,"Media commands:\n");
...
fprintf(stderr,"-X0xoff     (default 0)    x-offset in CALPLOT units\n");
fprintf(stderr,"-Y0yoff     (default 0)    y-offset in CALPLOT units\n");
