...
void dtv_fillpr(INT n, INT *x, INT *y); 
/*
...
static int determinant( int p1, int p2, int p3, INT *x, INT *y ) ;
static int no_interior( int p1, int p2, int p3, INT *x, INT *y, int *vp, int n, int poly_or ) ;
static int orientation( INT n, INT *x, INT *y ) ;
static int poly_clip(INT n, INT *x, INT *y) ;
static int succ(int b) ;
static void clipPoint(wcPt2 p, int b) ;
static void closeClip(void) ;
...
NOTE THIS FIXES THE plotxvig -R SHADED AREA but breaks plotnps shadep
also now still messed up for the N120E.PLT for plotxvig
*/
		if(dc_rotate){
			tmp = x[i];
			x[i] = y[i] ;
...
void dtv_fillp(INT n, INT *x, INT *y)
{
	...
	/* OK here turn into triangles and do triangle fill */
		...
		draw_poly(n,x,y);
		/* 27 OCT 2011 - TO GET THIS TO WORK WITH
		plotgif -R I had to remove the poly_clip
		since this messed up the mapping and
		actually clipped the image.  The low
		level line clip is done in dc_rliner in 
		plotgif.c - this is a major inconsistency
		and this is a hack */
		/* do the polygon clipping */ 
/*
		nout = poly_clip(n,x,y); 
...
static int no_interior( int p1, int p2, int p3, INT *x, INT *y, int *vp, int n, int poly_or )
/*    int p1, int p2, int p3;	 The vertices to consider */
/*    INT x[], INT y[];		 The vertex list */
