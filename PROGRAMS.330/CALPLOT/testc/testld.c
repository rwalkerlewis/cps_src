...
int main(void)
{
	...
	for(j=1;j<=3;j++){
		if(j == 1){
			...
			lined(x,c,100,1,0,0,ipat,xlen);
			lined(x,s,100,1,-10,1,ipat,xlen);
		} else if(j == 2){
			plot(0.0,-2.0,-3);
			...
			lined(x,c,50,2,-2,2,ipat,xlen);
			lined(x,s,50,2,0,0,ipat,xlen);
		} else if(j == 3){
			plot(0.0,-2.0,-3);
			gscale(x,xaxlen,100,1);
			...
			lined(x,cc,100,1,0,0,ipat,xlen);
			lined(x,ss,100,1,-10,1,ipat,xlen);
		}
	}
	pend();
	return 0;
